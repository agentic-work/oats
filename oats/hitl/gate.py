...
class ApprovalHandler(ABC):
    ...
    @abstractmethod
    def format_for_display(self, request: ApprovalRequest) -> Any:
        """
        Format the request for human-readable display.

        Returns a displayable object (str, Rich Panel, etc.) that clearly shows:
...
class CLIApprovalHandler(ApprovalHandler):
    ...
    def format_for_display(self, request: ApprovalRequest) -> str:
        ...
        lines = [
            "",
            "=" * 60,
            f"  OATS Tool Approval Request {risk_indicator}",
            "=" * 60,
            "",
