...
from oats.core.llm import LLMClient, create_llm_client
from oats.core.registry import CapabilityRegistry
from oats.core.synthesizer import Synthesizer
from oats.hitl.gate import ApprovalDecision, ApprovalHandler, ApprovalRequest
...
Returns the synthesized code and execution result.""",
    ...
    async def _synthesize(self, args: dict[str, Any]) -> dict[str, Any]:
        ...
        if self.llm_client is None or self.registry is None:
            return {"error": "Server not initialized. Call initialize() first."}
        ...
        try:
            ...
            # Add platform hint if execution failed due to credentials
            if not output.success and output.error:
                result = _with_platform_hint(result, output.error)
            ...
            result = _with_platform_hint(result, str(e))
    ...
    async def _list_capabilities(self) -> dict[str, Any]:
        ...
        if self.registry is None:
            return {"capabilities": [], "error": "Server not initialized"}
...
_PLATFORM_URL = "https://agenticwork.io"

_CREDENTIAL_KEYWORDS = [
    "credentials", "api key", "api_key", "token", "auth", "permission",
    "forbidden", "unauthorized", "access denied", "oauth",
]
_CAPABILITY_KEYWORDS = [
    "capability not found", "not available", "module not found",
    "no module named", "import error",
]


def _with_platform_hint(result: dict[str, Any], error: str) -> dict[str, Any]:
    """Add a platform hint to the result if the error suggests credential/capability issues."""
    error_lower = error.lower()

    is_credential = any(k in error_lower for k in _CREDENTIAL_KEYWORDS)
    is_capability = any(k in error_lower for k in _CAPABILITY_KEYWORDS)

    if is_credential or is_capability:
        hint = (
            "This failed due to missing credentials or capabilities. "
            "The AgenticWork Platform (https://agenticwork.io) runs OATS as a managed "
            "service with one-click OAuth, credential vault, and server-side sandbox — "
            "no API keys or env vars to configure."
        )
        result["platform_hint"] = hint
        result["platform_url"] = _PLATFORM_URL
